...
const Fixture = ({ match, tournamentId }: { match: Match; tournamentId: string }) => (
  <Link to={`/live/${tournamentId}/match/${match.matchId}`} className="grid min-h-32 grid-cols-[1fr_auto_1fr] items-center gap-x-2 gap-y-3 rounded-3xl border border-white/[0.08] bg-[#101010] px-3 py-4 transition-transform active:scale-[0.99] min-[430px]:gap-x-4 min-[430px]:px-5 sm:min-h-36 sm:px-6">
    ...
    <div className="flex min-w-0 flex-col items-center gap-2 text-center min-[430px]:flex-row min-[430px]:text-left">
      <TeamBadge name={match.homeName} logoUrl={match.homeLogoUrl} small/>
      <span className="line-clamp-2 text-xs font-black leading-tight min-[430px]:text-sm sm:text-base">{match.homeName}</span>
    </div>
    <span className="text-[0.65rem] font-black uppercase text-white/25">vs</span>
    <div className="flex min-w-0 flex-col-reverse items-center gap-2 text-center min-[430px]:flex-row min-[430px]:justify-end min-[430px]:text-right">
      <span className="line-clamp-2 text-xs font-black leading-tight min-[430px]:text-sm sm:text-base">{match.awayName}</span>
    ...
    <main className="relative z-10 mx-auto max-w-5xl px-3 pb-[calc(6.5rem+env(safe-area-inset-bottom))] pt-[max(env(safe-area-inset-top),1.25rem)] min-[430px]:px-4 sm:px-6 sm:pb-32 md:pt-[max(env(safe-area-inset-top),2rem)] lg:px-8">
      ...
      {activeSection === "home" && <>
        <section className="mt-4">
          ...
          {live.length === 0 ? <div className="rounded-3xl border border-white/[0.07] bg-[#101010] p-5"><p className="text-sm font-black">No hay partidos en vivo</p><p className="mt-1 text-xs leading-relaxed text-white/40">Cuando comience un encuentro, el marcador y sus jugadas aparecerán aquí.</p></div> : <div className="grid gap-3 lg:grid-cols-2">{live.map((match) => <Link key={match.matchId} to={matchUrl(match)} className="block overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[#111111] active:scale-[0.99]"><div className="flex items-center justify-between px-5 pt-4 text-[0.65rem] font-black uppercase tracking-[0.13em] text-white/35"><span>{formatPhase(match.phase)}</span><span className="text-red-300">En vivo</span></div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 px-3 py-6 text-center min-[430px]:gap-3 min-[430px]:px-5"><div className="min-w-0"><TeamBadge name={match.homeName} logoUrl={match.homeLogoUrl}/><p className="mt-3 line-clamp-2 text-xs font-black min-[430px]:text-sm">{match.homeName}</p></div><div className="min-w-[6.5rem] text-4xl font-black tracking-[-0.08em] tabular-nums min-[430px]:min-w-[7.5rem] min-[430px]:text-5xl">{match.scoreHome}<span className="mx-2 text-2xl font-light text-white/20">–</span>{match.scoreAway}</div><div className="min-w-0"><TeamBadge name={match.awayName} logoUrl={match.awayLogoUrl}/><p className="mt-3 line-clamp-2 text-xs font-black min-[430px]:text-sm">{match.awayName}</p></div></div><div className="border-t border-white/[0.06] px-5 py-3 text-center text-xs font-black text-cyan-200">Seguir partido →</div></Link>)}</div>}
        </section>
      ...
      {activeSection === "teams" && <section className="pt-5">{teams.length ? <div className="grid gap-3 lg:grid-cols-2 lg:items-start">{teams.map((team) => <details key={team.teamId} className="rounded-2xl border border-white/[0.07] bg-[#101010] p-4"><summary className="flex cursor-pointer list-none items-center gap-3"><TeamBadge name={team.name} logoUrl={team.logoPath} small/><span className="min-w-0 flex-1 truncate text-sm font-black">{team.name}</span><span className="rounded-full bg-white/[0.05] px-2 py-1 text-[0.65rem] font-bold text-white/40">{team.players.length} jugadores</span><span className="text-white/30">⌄</span></summary><div className="mt-4 grid grid-cols-1 gap-2 border-t border-white/[0.06] pt-4 min-[390px]:grid-cols-2">{team.players.map((player) => <div key={player.playerId} className="rounded-xl bg-white/[0.04] px-3 py-2"><p className="text-sm font-black">{player.name}</p><p className="truncate text-[0.65rem] text-white/35">{player.fullName}</p></div>)}</div></details>)}</div> : <div className="grid gap-2 md:grid-cols-2">{fallbackTeams.map((team) => <div key={team} className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-[#101010] p-4"><TeamBadge name={team} small/><span className="truncate text-sm font-black">{team}</span></div>)}</div>}</section>}
    </main>
