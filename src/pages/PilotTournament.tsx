...
const Fixture = ({ match, tournamentId }: { match: Match; tournamentId: string }) => <Link to={`/live/${tournamentId}/match/${match.matchId}`} className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-2xl border border-white/[0.07] bg-[#101010] px-4 py-4"><span className="col-span-3 -mb-1 text-center text-[0.58rem] font-black uppercase tracking-[0.14em] text-cyan-300/70">{stageName(match)}</span><div className="flex min-w-0 items-center gap-2"><TeamBadge name={match.homeName} logoUrl={match.homeLogoUrl} small/><span className="truncate text-sm font-black">{match.homeName}</span></div><span className="text-[0.65rem] font-black uppercase text-white/25">vs</span><div className="flex min-w-0 items-center justify-end gap-2 text-right"><span className="truncate text-sm font-black">{match.awayName}</span><TeamBadge name={match.awayName} logoUrl={match.awayLogoUrl} small/></div></Link>;
...
const PilotTournament = () => {
  ...
  if (loading) return <div className="min-h-screen bg-[#070707] px-4 py-6 text-white"><main className="mx-auto max-w-lg animate-pulse space-y-5"><div className="h-20 rounded-2xl bg-white/[0.05]"/><div className="h-64 rounded-[2rem] bg-white/[0.05]"/><div className="h-32 rounded-3xl bg-white/[0.05]"/></main></div>;
  if (error) return <div className="min-h-screen bg-[#070707] p-8 text-center font-semibold text-red-300">{error}</div>;

  const [eyebrow, heading] = sectionTitles[activeSection];
  const matchUrl = (match: Match) => `/live/${tournamentId}/match/${match.matchId}`;

  return <div className="champions-shell min-h-screen text-white">
    <main className="relative z-10 mx-auto max-w-lg px-4 pb-28 pt-5">
      <header className="pb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {activeSection !== "home" && <p className="mt-1 truncate text-xs font-bold uppercase tracking-[0.13em] text-white/35">{displayName}</p>}
          </div>
      ...
      {activeSection === "home" && <>
        <section className="mt-4">
          ...
          {live.length === 0 ? <div className="rounded-3xl border border-white/[0.07] bg-[#101010] p-5"><p className="text-sm font-black">No hay partidos en vivo</p><p className="mt-1 text-xs leading-relaxed text-white/40">Cuando comience un encuentro, el marcador y sus jugadas aparecerán aquí.</p></div> : <div className="space-y-3">{live.map((match) => <Link key={match.matchId} to={matchUrl(match)} className="block overflow-hidden rounded-[2rem] border border-white/[0.08] bg-[#111111] active:scale-[0.99]"><div className="flex items-center justify-between px-5 pt-4 text-[0.65rem] font-black uppercase tracking-[0.13em] text-white/35"><span>{formatPhase(match.phase)}</span><span className="text-red-300">En vivo</span></div><div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 px-5 py-6 text-center"><div className="min-w-0"><TeamBadge name={match.homeName} logoUrl={match.homeLogoUrl}/><p className="mt-3 truncate text-sm font-black">{match.homeName}</p></div><div className="min-w-[7.5rem] text-5xl font-black tracking-[-0.08em] tabular-nums">{match.scoreHome}<span className="mx-2 text-2xl font-light text-white/20">–</span>{match.scoreAway}</div><div className="min-w-0"><TeamBadge name={match.awayName} logoUrl={match.awayLogoUrl}/><p className="mt-3 truncate text-sm font-black">{match.awayName}</p></div></div><div className="border-t border-white/[0.06] px-5 py-3 text-center text-xs font-black text-cyan-200">Seguir partido →</div></Link>)}</div>}
        </section>
      ...
      {activeSection === "matches" && <>
        <section className="pt-5"><div className="mb-3 flex items-center justify-between px-1"><h2 className="text-base font-black">Próximos partidos</h2><span className="text-xs font-bold text-white/30">{upcoming.length} pendientes</span></div>{upcoming.length ? <div className="space-y-2">{upcoming.map((match) => <Fixture key={match.matchId} match={match} tournamentId={tournamentId}/>)}</div> : <div className="rounded-3xl border border-white/[0.07] bg-[#101010] p-5 text-sm text-white/40">No hay próximos partidos confirmados.</div>}</section>
      ...
      {activeSection === "stats" && <section className="pt-5">
        ...
        {playerStats.length ? <div className="overflow-hidden rounded-3xl border border-white/[0.07] bg-[#101010]">
          ...
          {playerStats.map((player, index) => <div key={player.playerId} className={`grid grid-cols-[2rem_1fr_2.5rem_2.5rem_3rem] items-center gap-2 px-4 py-3 text-sm ${index ? "border-t border-white/[0.05]" : ""}`}><span className="font-black text-white/30">{index + 1}</span><span className="truncate font-black">{player.name}</span><span className="text-center font-black">{player.goals}</span><span className="text-center text-cyan-200">{player.assists}</span><span className="text-center text-xs">{player.yellow ? `🟨${player.yellow}` : ""}{player.red ? ` 🟥${player.red}` : ""}</span></div>)}
      ...
      {activeSection === "teams" && <section className="pt-5">{teams.length ? <div className="space-y-2">{teams.map((team) => <details key={team.teamId} className="rounded-2xl border border-white/[0.07] bg-[#101010] p-4"><summary className="flex cursor-pointer list-none items-center gap-3"><TeamBadge name={team.name} logoUrl={team.logoPath} small/><span className="min-w-0 flex-1 truncate text-sm font-black">{team.name}</span><span className="rounded-full bg-white/[0.05] px-2 py-1 text-[0.65rem] font-bold text-white/40">{team.players.length} jugadores</span><span className="text-white/30">⌄</span></summary><div className="mt-4 grid grid-cols-2 gap-2 border-t border-white/[0.06] pt-4">{team.players.map((player) => <div key={player.playerId} className="rounded-xl bg-white/[0.04] px-3 py-2"><p className="text-sm font-black">{player.name}</p><p className="truncate text-[0.65rem] text-white/35">{player.fullName}</p></div>)}</div></details>)}</div> : <div className="space-y-2">{fallbackTeams.map((team) => <div key={team} className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-[#101010] p-4"><TeamBadge name={team} small/><span className="truncate text-sm font-black">{team}</span></div>)}</div>}</section>}
    </main>

    <nav aria-label="Navegación del torneo" className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[0.08] bg-[#0b0b0b]/95 px-2 pb-[max(env(safe-area-inset-bottom),0.45rem)] pt-2 backdrop-blur-xl sm:bottom-4 sm:mx-auto sm:max-w-lg sm:rounded-2xl sm:border">
      <div className="mx-auto grid max-w-lg grid-cols-5 gap-1">{sections.map((item) => <NavLink key={item.key} to={item.key === "home" ? `/live/${tournamentId}` : `/live/${tournamentId}/${item.path}`} end={item.key === "home"} className={({ isActive }) => `flex min-w-0 flex-col items-center gap-1 rounded-xl px-1 py-2 text-[0.6rem] font-black transition-colors ${isActive ? "bg-cyan-300/[0.1] text-cyan-200" : "text-white/40"}`}><NavIcon name={item.key}/><span className="truncate">{item.label}</span></NavLink>)}</div>
    </nav>
  </div>;
};
