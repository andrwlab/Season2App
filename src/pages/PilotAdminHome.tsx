    ...
    return (
      <div className="min-h-screen bg-slate-950 px-4 py-10 text-white">
        <main className="mx-auto max-w-md space-y-5">
          <div><p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-300">Tournament Admin</p><h1 className="mt-2 text-3xl font-black">Tournament Manager</h1><p className="mt-2 text-sm text-slate-400">Create a tournament or return to one you already configured.</p></div>
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-6 text-white">
      <main className="mx-auto max-w-2xl space-y-6">
        <header className="flex items-start justify-between gap-4">
          <div><p className="text-[0.65rem] font-black uppercase tracking-[0.25em] text-cyan-300">Tournament Admin</p><h1 className="mt-2 text-3xl font-black">Tournament Manager</h1><p className="mt-1 text-sm text-slate-500">One place to create, open and manage every pilot tournament.</p></div>
        ...
        <form onSubmit={createTournament} className="space-y-4 rounded-3xl border border-cyan-300/20 bg-cyan-300/[0.05] p-5">
          <div><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-300">New tournament</p><p className="mt-1 text-sm text-slate-400">Create it here instead of manually changing the URL.</p></div>
        ...
        <section>
          <div className="mb-3 flex items-center justify-between"><h2 className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Your tournaments</h2><span className="text-xs font-bold text-slate-500">{tournaments.length}</span></div>
              ...
              {tournaments.map((item) => (
                <div key={item.tournamentId} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 flex-col gap-2 sm:flex-row"><Link to={`/pilot/${item.tournamentId}/setup`} className="rounded-lg bg-slate-800 px-3 py-2 text-center text-xs font-black">MANAGE</Link><Link to={`/live/${item.tournamentId}`} className="rounded-lg bg-cyan-300/10 px-3 py-2 text-center text-xs font-black text-cyan-200">PUBLIC HUB</Link></div>
                  </div>
                </div>
              ))}
            </div>
